...
---
# 在Azure HDInsight上使用Spark的数据u科学概述
[!INCLUDE [machine-learning-spark-modeling](../../includes/machine-learning-spark-modeling.md)]
这一系列话题将演示如何利用HDInsight Spark来完成常见的数据科学任务，例如数据摄取，特征工程，建模和模型评估。所使用的数据是2013年纽约出租车行程和票价数据集的样本。 建立的模型包括逻辑和线性回归，随机森林和梯度提升树。 还展示了如何将这些模型存储在Azure blob存储（WASB）中以及如何对其预测性能进行评分和评估。更高级的话题包含了如何使用交叉验证和超参数扫描来训练模型。此概述还引用了描述如何设置Spark集群的话题，您通过完成提供的演练中的步骤即可。
...
## Spark 和 MLlib
[Spark](http://spark.apache.org/)是一个开源的并行处理框架，支持内存中处理以提高大数据分析应用程序的性能。Spark处理引擎是为了速度、易用性和复杂的分析而构建的。 
...
- [pySpark-machine-learning-data-science-spark-model-consumption.ipynb](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/Spark/pySpark/pySpark-machine-learning-data-science-spark-model-consumption.ipynb): Shows how to operationalize a saved model using Python on HDInsight clusters.
